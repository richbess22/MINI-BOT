    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        try {
            ...
            if (config.AUTO_VIEW_STATUS === 'true') {
                let retries = config.MAiX_RETRIES;
                while (retries > 0) {
                    try {
        ...
        try {
            switch (command) {   
                case 'alive': {
                    ...
                    const title = '*ᴍᴜʟʟᴇʀ ᴍɪɴɪ ʙᴏᴛ ᴀʟɪᴠᴇ ɴᴏᴡ 😾❤*';
                    const content = `*ᴍᴜʟʟᴇʀ ᴍɪɴɪ ʙᴏᴛ ʙʏ ᴍᴜʟʟᴇʀ  ᴛᴇᴄʜ*\n` +                                   `*ʙᴏᴛ ᴏᴡɴᴇʀ :- ᴍᴜʟʟᴇʀ  ᴛᴇᴄʜ*\n` +
                                `*ʙᴏᴛ ɴᴀᴍᴇ :- ᴍᴜʟʟᴇʀ ᴍɪɴɪ ʙᴏᴛ ᴠ1*\n` +
                                   `*ʙᴏᴛ ᴡᴇʙ ꜱɪᴛᴇ*\n` +
                                   `> *ᴄᴏᴍɪɴɢ ꜱᴏᴏɴ*`;
...
case 'menu': {
    ...
    const title = 'ᴍᴜʟʟᴇʀ ᴍɪɴɪ ʙᴏᴛ ᴍᴇɴᴜ 😾❤*';
    const text = `╭──➢\n` +
        `│ \`S T A T U S\`\n` +
        `│ *⦁ ʙᴏᴛ ɴᴀᴍᴇ*: ᴍᴜʟʟᴇʀ ᴍɪɴɪ ʙᴏᴛ\n` +
        `│ *⦁ ʙᴏᴛ ᴏᴡɴᴇʀ*:ᴍᴜʟʟᴇʀ  ᴛᴇᴄʜ\n` +
        `│ *⦁ ᴠᴇʀꜱɪᴏɴ*: 0.0001+\n` +
        `│ *⦁ ᴘʟᴀᴛꜰᴏᴇᴍ*: Heroku\n` +
...
//=======================================
                ...
                case 'owner': {
                    const vcard = 'BEGIN:VCARD\n'
                        ...
                        + 'FN:SHALA OWNER\n'
                        + 'ORG:SHALA OWNER\n'
                        + 'TEL;type=CELL;type=VOICE;waid=2349166255902:+2349166255902\n'
                        + 'EMAIL:mullertech@gmail.com\n'
                        + 'END:VCARD';

                    await socket.sendMessage(sender, {
                        contacts: {
                            displayName: "MULLER OWNER",
                            contacts: [{ vcard }]
                        },
                        image: { url: config.BUTTON_IMAGES.OWNER },
                        caption: '*👨‍💻 MULLER BOT OWNER DETAILS*',
                        buttons: [
                            { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📋 MENU' }, type: 1 },
                ...
                case 'song': {
                    try {
                        ...
                        const titleText = '*MULLER SONG DOWNLOAD*';
                        const content = `┏━━━━━━━━━━━━━━━━\n` +
                            `┃📝 \`Title\` : ${video.title}\n` +
                            `┃📈 \`Views\` : ${video.views}\n` +
